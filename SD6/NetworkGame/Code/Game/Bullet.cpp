...
{
	for (std::vector<Entity*>::iterator it = g_theGame->m_clientWorld->m_entities.begin(); it != g_theGame->m_clientWorld->m_entities.end(); ++it)
	{
		Entity* ent = *it;
		if (ent == this)
		{
	...
	if (m_isEnemy)
	{
		Player* player = g_theGame->m_clientWorld->m_myPlayer;
