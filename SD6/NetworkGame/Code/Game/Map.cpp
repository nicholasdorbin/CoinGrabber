...
Map::Map(const std::string& imageFilePath)
{
	...
	else
	{
		...
		for (int texelIndex = 0; texelIndex < numOfTexels; texelIndex++)
		{
			...
			Rgba texel((unsigned char)r, (unsigned char)g, (unsigned char)b, (unsigned char)a);
		...
		for (int j = inputTexelSize.y - 1; j >= 0; j--)
		{
			...
			for (int i = 0; i <= inputTexelSize.x - 1; i++)
			{
				...
				int x = i;
				x = (int)RangeMap((float)i, 0.f, (float)inputTexelSize.x, leftBounds, rightBounds);

				int y = j;
				y = (int)RangeMap((float)j, 0.f, (float)inputTexelSize.y, bottomBounds, topBounds);

				Vector2 pos = Vector2((float)x, topBounds + 0.01f);
...
Map::~Map()
{
	for each (MapRow row in m_rowData)
	{
