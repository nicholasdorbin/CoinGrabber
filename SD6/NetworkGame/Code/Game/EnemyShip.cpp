...
bool EnemyShip::CanSeePlayer()
{
	...
	Vector2 start = m_pos;
	Vector2 end = g_theGame->m_clientWorld->m_myPlayer->m_pos;
...
void EnemyShip::Update(const float deltaSeconds)
{
	...
	bool isPlayerAlive = g_theGame->m_clientWorld->m_myPlayer->m_health > 0 ? true : false;
...
// 		g_theGame->m_audio->PlaySound(sound, 1.f);
	...
	Vector2 playerPos = g_theGame->m_clientWorld->m_myPlayer->m_pos;
	float playerRadius = g_theGame->m_clientWorld->m_myPlayer->m_physicsRadius;
	if (DoDiscsOverlap(m_pos, m_physicsRadius, playerPos, playerRadius))
	{
		m_isDead = true;
		g_theGame->m_clientWorld->m_myPlayer->m_health -= m_collisionDamage;
	}
	...
	if (distToPlayer <= MAX_DISTANCE_TO_PLAYER)
	{
		if (!m_isDead)
		{
			...
			if (m_isShooting && m_nextShootTick < m_age)
			{
				...
				g_theGame->m_clientWorld->m_entitiesToAdd.push_back(newBullet);
